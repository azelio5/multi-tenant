    ...
    public String resolveTenantSchema(final String tenantId) {
        ...
        try {
            final String companyCode = this.jdbcTemplate
                    .queryForObject("SELECT company_code FROM public.tenants WHERE id = ? " +
                            "AND deleted = false", String.class, tenantId);
            if (companyCode != null) {
            ...
            log.warn("tenant schema resolution error for tenant {}, using public schema", tenantId);
            return PUBLIC_SCHEMA;
        }
